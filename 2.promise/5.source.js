...
const regeneratorRuntime = {
    ...
    wrap(innerFunc,outerFn){
        ...
        return {
            next(data){ // 每次调用next 都会执行这个read$函数 inerFunc
...
// polyfill
function read() { // generator 最终被编译出来了 这个方法
  var out1, out2;
  return regeneratorRuntime.wrap(function read$(_context) {
    while (1) { // 在编写代码的时候 如果一个函数 会执行多次 我们通常会用一个while(1) 来表识代码会走多次
      switch (_context.prev = _context.next) { // next -》 prev 判断的是prev
